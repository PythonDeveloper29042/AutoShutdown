...
def shutdown_model(time: datetime, **countdown_data):
    ...
    if time.strip() == '':
        messagebox.showerror('错误', 'ERROR 1: 选择的时间不能为空，请重选')  # Show error message if time is empty
        return
    ...
    countdown_data['end_time'].set(f'系统将于以下时间关机：\n{current_time.strftime("%Y-%m-%d %H:%M:%S")}')  # Set the end time in the countdown data
...
def shutdown_custom(time: datetime, **countdown_data):
    ...
    if time <= current_time:  # Check if the selected time is in the past
        messagebox.showerror('错误', 'ERROR 2: 选择的时间不能早于当前时间，请重选')  # Show error message if the selected time is in the past
        return
    ...
    countdown_data['end_time'].set(f'系统将于以下时间关机：\n{time.strftime("%Y-%m-%d %H:%M:%S")}')  # Set the end time in the countdown data
    th = Thread(target=countdown, args=(countdown_data,))  # Create a new thread for the countdown function
    th.start()  # Start the thread
...
def cancel_shutdown():
    ...
    messagebox.showinfo('Info', '操作已被用户取消')  # Show a message box to inform the user that the shutdown has been cancelled
